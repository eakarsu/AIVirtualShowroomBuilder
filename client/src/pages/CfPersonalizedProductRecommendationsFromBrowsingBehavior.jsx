...
export default function CfPersonalizedProductRecommendationsFromBrowsingBehavior() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-personalized-product-recommendations-from-browsing-behavior/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-personalized-product-recommendations-from-browsing-behavior', project: 'AIVirtualShowroomBuilder' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-personalized-product-recommendations-from-browsing-behavior/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Personalized product recommendations from browsing behavior</h1>
