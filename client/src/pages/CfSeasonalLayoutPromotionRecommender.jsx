...
// Feature: Seasonal layout/promotion recommender
...
export default function CfSeasonalLayoutPromotionRecommender() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-seasonal-layout-promotion-recommender/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-seasonal-layout-promotion-recommender', project: 'AIVirtualShowroomBuilder' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-seasonal-layout-promotion-recommender/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Seasonal layout/promotion recommender</h1>
