...
// Feature: Visual merchandising optimizer using conversion data
...
export default function CfVisualMerchandisingOptimizerUsingConversionData() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-visual-merchandising-optimizer-using-conversion-data/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-visual-merchandising-optimizer-using-conversion-data', project: 'AIVirtualShowroomBuilder' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-visual-merchandising-optimizer-using-conversion-data/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Visual merchandising optimizer using conversion data</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AIVirtualShowroomBuilder</p>
      </div>
