...
// Feature: Customer journey heatmap visualizing dwell time
...
export default function CfCustomerJourneyHeatmapVisualizingDwellTime() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-customer-journey-heatmap-visualizing-dwell-time/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-customer-journey-heatmap-visualizing-dwell-time', project: 'AIVirtualShowroomBuilder' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-customer-journey-heatmap-visualizing-dwell-time/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Customer journey heatmap visualizing dwell time</h1>
