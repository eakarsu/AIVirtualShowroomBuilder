...
// Competitor showroom analysis adapting layouts
// Feature: competitor-showroom-analysis
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'competitor-showroom-analysis', payload, result);
    res.json({ success: true, feature: 'competitor-showroom-analysis', ...result });
  } catch (err) {
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Competitor showroom analysis adapting layouts. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'competitor-showroom-analysis', title: 'Competitor showroom analysis adapting layouts', project: 'AIVirtualShowroomBuilder' });
});
