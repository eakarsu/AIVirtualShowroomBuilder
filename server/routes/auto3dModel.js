...
// 3D model auto-generation from product photos
// Feature: auto-3d-model
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'auto-3d-model', payload, result);
    res.json({ success: true, feature: 'auto-3d-model', ...result });
  } catch (err) {
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: 3D model auto-generation from product photos. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'auto-3d-model', title: '3D model auto-generation from product photos', project: 'AIVirtualShowroomBuilder' });
});
