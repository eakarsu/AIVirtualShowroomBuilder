...
// Seasonal layout/promotion recommender
// Feature: seasonal-layout-recommender
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'seasonal-layout-recommender', payload, result);
    res.json({ success: true, feature: 'seasonal-layout-recommender', ...result });
  } catch (err) {
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Seasonal layout/promotion recommender. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'seasonal-layout-recommender', title: 'Seasonal layout/promotion recommender', project: 'AIVirtualShowroomBuilder' });
});
