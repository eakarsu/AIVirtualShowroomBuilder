...
// Visual merchandising optimizer using conversion data
// Feature: visual-merchandising-optimizer
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'visual-merchandising-optimizer', payload, result);
    res.json({ success: true, feature: 'visual-merchandising-optimizer', ...result });
  } catch (err) {
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Visual merchandising optimizer using conversion data. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'visual-merchandising-optimizer', title: 'Visual merchandising optimizer using conversion data', project: 'AIVirtualShowroomBuilder' });
});
