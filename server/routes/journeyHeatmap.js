...
// Customer journey heatmap visualizing dwell time
// Feature: journey-heatmap
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'journey-heatmap', payload, result);
    res.json({ success: true, feature: 'journey-heatmap', ...result });
  } catch (err) {
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Customer journey heatmap visualizing dwell time. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'journey-heatmap', title: 'Customer journey heatmap visualizing dwell time', project: 'AIVirtualShowroomBuilder' });
});
